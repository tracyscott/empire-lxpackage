...
import org.projectempire.lx.wavetable.PerlinWavetable;

@LXCategory("Empire")
public class CosPerlinPolar extends LXPattern {
    ...
    public final EnumParameter<Direction> direction =
            new EnumParameter<>("Direction", Direction.ANGLE)
                    .setDescription("Whether to map by angle or radius");
    ...
    public final CompoundParameter paletteScale =
            new CompoundParameter("P.Scale", 1.0, 0.0, 10.0)
                    .setDescription("Scale factor for palette sampling range (0-1)");
    
    public final CompoundParameter swirl =
            new CompoundParameter("Swirl", 1.0, -20.0, 20.0)
                    .setDescription("Scale factor for angle as a function of radius to create swirls");
    
    public final CompoundParameter perlinPos =
            new CompoundParameter("P.Pos", 0.0, 0.0, 1.0)
                    .setDescription("Position offset for Perlin noise wavetable");
    
    public final CompoundParameter perlinFreq =
            new CompoundParameter("P.Freq", 1.0, 0.1, 1000.0)
                    .setDescription("Frequency scale for Perlin noise generation");
    
    public final CompoundParameter perlinWidth =
            new CompoundParameter("P.Width", 1.0, 0.1, 5.0)
                    .setDescription("Width scaling for Perlin noise wavetable sampling");
    
    private CosPalette cosPalette;
    private PerlinWavetable perlinWavetable;
    private double lastPerlinFreq = -1;
    
    public CosPerlinPolar(LX lx) {
        super(lx);
        ...
        addParameter("swirl", this.swirl);
        addParameter("pPos", this.perlinPos);
        addParameter("pFreq", this.perlinFreq);
        addParameter("pWidth", this.perlinWidth);
        ...
        this.perlinWavetable.generateWavetable(1f, 0f);
        this.perlinWavetable.mirror();
        this.lastPerlinFreq = this.perlinFreq.getValue();
    }
    ...
    protected void run(double deltaMs) {
        // Check if Perlin frequency has changed and rebuild wavetable if necessary
        if (this.perlinFreq.getValue() != this.lastPerlinFreq) {
            this.perlinWavetable = new PerlinWavetable(1024, 1, (float) this.perlinFreq.getValue());
            this.perlinWavetable.generateWavetable(1f, 0f);
            this.perlinWavetable.mirror();
            this.lastPerlinFreq = this.perlinFreq.getValue();
        ...
        // Update Perlin wavetable position
        this.perlinWavetable.pos = (float) this.perlinPos.getValue();
        ...
        // Map each point's polar coordinate (with rotation and offset) to a color
        for (LXPoint point : this.model.points) {
            ...
            double radius = Math.sqrt(rotatedCoord1 * rotatedCoord1 + rotatedCoord2 * rotatedCoord2);
            double angle = Math.atan2(rotatedCoord2, rotatedCoord1);
            
            // Apply swirl effect: modify angle based on radius
            double swirlEffect = this.swirl.getValue() * radius;
            angle = angle + swirlEffect;
            
            // Normalize angle from -π to π to 0 to 1
            double normalizedAngle = (angle + Math.PI) / (2 * Math.PI);
            
            // Normalize radius (assuming max radius is √0.5 from center to corner)
            double maxRadius = Math.sqrt(0.5);
            double normalizedRadius = Math.min(1.0, radius / maxRadius);
            
            // Select the final coordinate based on direction
            Direction selectedDirection = this.direction.getEnum();
            double coord = (selectedDirection == Direction.ANGLE) ? normalizedAngle : normalizedRadius;
            ...
            // Get Perlin wavetable value for this point's position
            float perlinValue = this.perlinWavetable.getSample((float) coord, (float) this.perlinWidth.getValue());
            
            // Multiply RGB values by Perlin wavetable value
            this.colors[point.index] = LXColor.rgb(
                (int)(Math.max(0, Math.min(255, rgb[0] * 255 * perlinValue))),
                (int)(Math.max(0, Math.min(255, rgb[1] * 255 * perlinValue))),
                (int)(Math.max(0, Math.min(255, rgb[2] * 255 * perlinValue)))
            );
        }
    }
}