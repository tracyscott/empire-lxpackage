...
import org.projectempire.lx.vstrip.FlareletGen;
import org.projectempire.lx.vstrip.Topology;
import org.projectempire.lx.vstrip.LXUtil;
...
@LXCategory("Form")
@LXComponentName("EFlareletBang")
public class EFlareletBang extends LXPattern {
  ...
  protected FlareletGen flareletGen4;
  protected FlareletGen flareletGen5;

  public EFlareletBang(LX lx) {
    super(lx);
    LXUtil.setLX(lx);
    ...
    // Create a flarelet generator for each horn
    flareletGen1 = new FlareletGen(lx, Topology.getDefaultTopologies(lx).get(0));
    flareletGen1.setHornStartStripIds(Topology.pCenterHornStartStripIds);
    flareletGen4 = new FlareletGen(lx, Topology.getDefaultTopologies(lx).get(0));
    flareletGen4.setHornStartStripIds(Topology.pFrontHornStripIds);

    flareletGen2 = new FlareletGen(lx, Topology.getDefaultTopologies(lx).get(0));
    flareletGen2.setHornStartStripIds(Topology.dCenterHornStartStripIds);
    flareletGen5 = new FlareletGen(lx, Topology.getDefaultTopologies(lx).get(0));
    flareletGen5.setHornStartStripIds(Topology.dFrontHornStripIds);

    flareletGen3 = new FlareletGen(lx, Topology.getDefaultTopologies(lx).get(0));
    flareletGen3.setHornStartStripIds(Topology.cHornStripIds);
  }
    ...
    switch (which) {
      case 1:
        if (ring1.isOn()) {
          ...
          flareletGen4.startFlareletAll();
        } else {
          flareletGen1.startFlareletIncrement();
          flareletGen4.startFlareletIncrement();
        }
        break;
      case 2:
        if (ring2.isOn()) {
          flareletGen2.startFlareletAll();
          flareletGen5.startFlareletAll();
        } else {
          flareletGen2.startFlareletIncrement();
          flareletGen5.startFlareletIncrement();
        }
        break;
      case 3:
        if (ring3.isOn()) {
  ...
  public void run(double deltaMs) {
    ...
    flareletGen1.cloneAtJoints = true;
    flareletGen4.speed = speed1.getValuef();
    flareletGen4.fadeTime = fadeTime1.getValuef() * 1000f;
    flareletGen4.fx = (fxDepth1.getValuef() > 0.05)?1:0;
    flareletGen4.fxDepth = fxDepth1.getValuef();
    flareletGen4.waveWidth = widthKnob1.getValuef();
    flareletGen4.color = colorKnob1.getColor();
    flareletGen4.swatch = swatch1.getValuei();
    flareletGen4.pow = pow1.getValuef();
    flareletGen4.brt = brt1.getValuef();
    flareletGen4.bright = brt1.getValuef() > 0.001;
    flareletGen4.position = pos1.getValuef();
    flareletGen4.setWavetable(waveKnob1.getValuei());
    flareletGen4.cloneAtJoints = true;
    ...
    flareletGen2.cloneAtJoints = true;
    flareletGen5.speed = speed2.getValuef();
    flareletGen5.fadeTime = fadeTime2.getValuef() * 1000f;
    flareletGen5.fx = (fxDepth2.getValuef() > 0.05)?1:0;
    flareletGen5.fxDepth = fxDepth2.getValuef();
    flareletGen5.waveWidth = widthKnob2.getValuef();
    flareletGen5.color = colorKnob2.getColor();
    flareletGen5.swatch = swatch2.getValuei();
    flareletGen5.pow = pow2.getValuef();
    flareletGen5.brt = brt2.getValuef();
    flareletGen5.bright = brt2.getValuef() > 0.001;
    flareletGen5.position = pos2.getValuef();
    flareletGen5.setWavetable(waveKnob2.getValuei());
    flareletGen5.cloneAtJoints = true;
    ...
    flareletGen3.cloneAtJoints = true;

    flareletGen1.disposeExpiredFlarelets();
    flareletGen4.disposeExpiredFlarelets();
    flareletGen2.disposeExpiredFlarelets();
    flareletGen5.disposeExpiredFlarelets();
    ...
    for (Flarelet f : flareletGen1.runningFlarelets) {
      ...
      f.waveOnTopFan(colors, LXColor.Blend.ADD, 0, true);
      f.pos += speed1.getValuef() * (float)deltaMs / 1000f;
      if (!f.cloneRequests.isEmpty()) {
        for (Flarelet.CloneRequest cloneRequest : f.cloneRequests) {
          Flarelet clone = f.cloneFlarelet(cloneRequest.dStrip, cloneRequest.pos);
          clonesToAdd.add(clone);
        }
        f.cloneRequests.clear();
      }
    }
    flareletGen1.runningFlarelets.addAll(clonesToAdd);
    clonesToAdd.clear();

    for (Flarelet f : flareletGen4.runningFlarelets) {
      //lx.log("Rendering flare on 4.");
      f.waveOnTopFan(colors, LXColor.Blend.ADD, 0, true);
      f.pos += speed1.getValuef() * (float)deltaMs / 1000f;
      if (!f.cloneRequests.isEmpty()) {
        for (Flarelet.CloneRequest cloneRequest : f.cloneRequests) {
    ...
    flareletGen4.runningFlarelets.addAll(clonesToAdd);
    clonesToAdd.clear();
    ...
    flareletGen2.runningFlarelets.addAll(clonesToAdd);
    clonesToAdd.clear();

    for (Flarelet f : flareletGen5.runningFlarelets) {
    ...
    flareletGen5.runningFlarelets.addAll(clonesToAdd);
    clonesToAdd.clear();
    ...
    flareletGen3.runningFlarelets.addAll(clonesToAdd);
    clonesToAdd.clear();
  }
}