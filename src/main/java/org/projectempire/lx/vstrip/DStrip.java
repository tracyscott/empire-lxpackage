...
public class DStrip {
    ...
    public DStrip(VTopology vTop, int stripId, boolean forward) {
        ...
        //LX.log("Initializing DStrip with stripId: " + stripId + " forward: " + forward);
        if (vStrip == null) {
            //LX.error("DStrip: vStrip is null for stripId: " + stripId);
            return;
        }
    ...
    public int getNextStripJointCount() {
        if (forward) {
            return vStrip.myEndPointJoints.size();
        } else {
            return vStrip.myStartPointJoints.size();
        }
    }
    ...
    public DStrip chooseStripFromJoints(List<VJoint> joints, int jointSelector) {
        ...
        if (joints.size() == 0) {
            //LX.log("Joints null for: " + vStrip.id + " jointSelector: " + jointSelector);
            return null;
        }
        if (jointNum == -1)
            jointNum = ThreadLocalRandom.current().nextInt(joints.size());
        if (jointNum >= joints.size()) jointNum = joints.size() - 1;
